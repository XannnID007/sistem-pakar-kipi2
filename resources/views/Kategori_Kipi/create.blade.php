...
@section('page_title', 'Tambah Kategori KIPI')

@section('content')

    <div class="max-w-2xl mx-auto bg-white p-8 rounded-3xl shadow-xl border border-slate-100">
        <h2 class="text-2xl font-bold text-slate-800 mb-8 pb-4 border-b border-slate-200">Tambah Kategori KIPI Baru</h2>
        ...
        <form action="{{ route('pakar.kategori_kipi.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <p class="mt-2 text-xs text-slate-500">Kode KIPI ini dihasilkan secara otomatis oleh sistem.</p>
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="jenis_kipi" id="jenis_kipi"
                    ...
                    value="{{ old('jenis_kipi') }}" placeholder="Contoh: Ringan (reaksi lokal)" required>
                @error('jenis_kipi')
            ...
            <div class="mb-8">
                ...
                <textarea name="saran" id="saran"
                    class="block w-full px-4 py-3 border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-300 text-slate-800 placeholder-slate-400 text-lg"
                    rows="4" placeholder="Masukkan saran penanganan..." required>{{ old('saran') }}</textarea>
                @error('saran')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
